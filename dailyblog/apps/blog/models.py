...
class Article(models.Model):
    ...
    #save article when using markdown editor,By far,it has been stopped
    def mark_save(self):
        self.content_html = markdown(self.content,output_format='html5')
        self.abstract = self.content[:200] if not self.abstract else self.abstract
    ...
    def get_latest_articles(cls,number=8):
        return cls.objects.filter(status=0).values('title','alias','pub_time','read_times').order_by('-pub_time')[:number]
